...
import { Radio, CheckCircle2, AlertTriangle, Zap, Lock, Activity, Thermometer } from 'lucide-react';
...
const schema = z.object({
  ...
  rms:      z.coerce.number({ invalid_type_error: 'RMS value is required' })
              .min(0, 'RMS cannot be negative')
              .max(50, 'RMS cannot exceed 50 mm/s — check your sensor'),
  temp:     z.coerce.number({ invalid_type_error: 'Temperature is required' })
              .min(-50, 'Temperature cannot be below -50 °C')
              .max(250, 'Temperature cannot exceed 250 °C — check your sensor'),
...
export function Simulator() {
  ...
  const mutation = useMutation({
    mutationFn: (d) => simulatorApi.publish(d),
    onSuccess: (reading) => {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <Card>
          ...
          >
            ...
            <Select
              options={sensors
                .filter(s => s.active && s.assetActive !== false)
                .map(s => ({ value: String(s.id), label: `${s.name} — ${s.serialNumber ?? 'no serial'} (${s.assetName})` }))}
